...
use cosmwasm_std::{
    ...
    Storage, Addr, Reply, Uint128, Uint64, to_json_binary
};
use cw2::set_contract_version;
use crate::error::ContractError;
use crate::msg::{InstantiateMsg, MigrateMsg, QueryMsg};
use crate::state::{LIMITS, Limit};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateLimit { index, limit } => handle_update_limit(deps, env, info, index, limit),
            ...
            Limit::Periodic(ref mut l) => {
                ...
                if block_time > l.end_period() {
                    ...
                    l.begin_period += (block_time - l.begin_period)
                        .checked_div_floor((l.periodic, Uint64::one())).unwrap()
                        .checked_mul(l.periodic).unwrap();
                }
...
fn handle_unregister(
    ...
    Ok(Response::new().add_attribute("action", "unregister")
...
fn handle_update_limit(
    ...
    Ok(Response::new().add_attribute("action", "edit_limit")
...
fn handle_delete_limit(
    ...
    Ok(Response::new().add_attribute("action", "delete_limit")
    .add_attribute("index", index.to_string()))
}
